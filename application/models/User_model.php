...
class User_model extends CI_model
{
    ...
    public function count_berkas_diklat()
    {
        $id_user =  $this->session->userdata('id');
        $query = $this->db->query(
            "SELECT COUNT(file_upload) as berkas_diklat
                               FROM berkas_diklat
                               WHERE id_user = $id_user"
        );
        if ($query->num_rows() > 0) {
            return $query->row()->berkas_diklat;
        } else {
            return 0;
        }
    }
    ...
    public function getpenghargaan()
    {
        $id_user =  $this->session->userdata('id');
        $this->db->select('*');
        $this->db->from('berkas_penghargaan');
        $this->db->where('id_user', $id_user);
        $this->db->order_by('id', 'DESC');
        $query = $this->db->get()->result_array();
        return $query;
    }
    public function getpenilaiankerja()
    {
        $id_user =  $this->session->userdata('id');
        $this->db->select('*');
        $this->db->from('berkas_penilaiankerja');
        $this->db->where('id_user', $id_user);
        $this->db->order_by('id', 'DESC');
        $query = $this->db->get()->result_array();
        return $query;
