...
class User extends CI_Controller
{
    ...
    public function berkas_diklat()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = 'Input Diklat';
            $data['user'] = $this->db->get_where('mst_user', ['username' => $this->session->userdata('username')])->row_array();
            $data['berkas_diklat'] = $this->get_laporan->getdiklat();

            $this->load->view('templates/topbar', $data);
            $this->load->view('templates/sidebar_user', $data);
            $this->load->view('user/berkas_diklat', $data);
            $this->load->view('templates/footer');
        } else {
            $upload_file = $_FILES['file']['name'];
            if ($upload_file) {
                $config['allowed_types'] = 'xls|xlsx|doc|docx|ppt|pptx|pdf|zip|rar';
                $config['max_size']     = '51200';
                $config['upload_path'] = './assets/files/berkas_diklat/';
                $this->load->library('upload', $config);
                if ($this->upload->do_upload('file')) {
                    $old_file = $data['id']['file'];
                    if ($old_file != 'default.docx') {
                        unlink(FCPATH . 'assets/files/berkas_diklat/' . $old_file);
                    }
                    $new_file = $this->upload->data('file_name');
                    $data = array(
                        'id_user' => $this->input->post('id_user'),
                    ...
                    $this->db->insert('berkas_diklat', $data);
                    $this->session->set_flashdata('message', 'Upload data');
                    redirect('user/berkas_diklat');
                } else {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger font-weight-bolder" role="alert">UPLOAD GAGAL !..  Ekstensi File Salah / Ukuran file tidak boleh dari 10 mb</div>');
                    redirect('user/berkas_diklat');
                }
    ...
    public function edit_berkas_diklat($id)
    {
        $this->form_validation->set_rules('id', 'ID', 'required|trim');

        if ($this->form_validation->run() == FALSE) {
            $data['title'] = 'Edit Berkas Diklat';
            $data['user'] = $this->db->get_where('mst_user', ['username' => $this->session->userdata('username')])->row_array();
            $data['berkas_diklat'] = $this->db->get_where('berkas_diklat', ['id' => $id])->row_array();

            $this->load->view('templates/topbar', $data);
            $this->load->view('templates/sidebar_user', $data);
            $this->load->view('user/edit_berkas_diklat', $data);
            $this->load->view('templates/footer');
        } else {
            $upload_file = $_FILES['file']['name'];
            if ($upload_file) {
                ...
                $this->load->library('upload', $config);
                if ($this->upload->do_upload('file')) {
                    $old_file = $data['berkas_diklat']['file_upload'];
                    if ($old_file != 'default.docx') {
                        unlink(FCPATH . 'assets/files/berkas_diklat/' . $old_file);
                    }
                    $new_file = $this->upload->data('file_name');
                    ...
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger font-weight-bolder" role="alert">UPLOAD GAGAL !! Ekstensi File Salah / Ukuran file tidak boleh dari 10 mb</div>');
                    redirect('user/berkas_diklat/' . $id);
                }
            }
            $id = $this->input->post('id');
            $date_edit = $this->input->post('date_edit');
            ...
            $this->db->set('nama_file', $nama_file);
            $this->db->where('id', $id);
            $this->db->update('berkas_diklat');

            $this->session->set_flashdata('message', 'Update data');
            redirect('user/edit_berkas_diklat/' . $id);
        }
    }

    public function file_download_berkas_diklat($id)
    {
        $data = $this->db->get_where('berkas_diklat', ['id' => $id])->row_array();
        header("Content-Disposition: attachment; filename=" . $data['file_upload']);
        $fp = fopen("assets/files/berkas_diklat/" . $data['file_upload'], 'r');
        $content = fread($fp, filesize('assets/files/berkas_diklat/' . $data['file_upload']));
        fclose($fp);
        echo $content;
        exit;
    }

    public function del_berkas_diklat($id)
    {
        $_id = $this->db->get_where('berkas_diklat', ['id' => $id])->row();
        $query = $this->db->delete('berkas_diklat', ['id' => $id]);
        if ($query) {
            unlink("assets/files/berkas_diklat/" . $_id->file_upload);
        }
        $this->session->set_flashdata('message', 'Hapus data');
        redirect('user/berkas_diklat');
    }

    public function berkas_penghargaan()
    {
        $this->form_validation->set_rules('id_user', 'ID', 'required|trim');

        if ($this->form_validation->run() == false) {
            $data['title'] = 'Input Penghargaan';
            $data['user'] = $this->db->get_where('mst_user', ['username' => $this->session->userdata('username')])->row_array();
            $data['berkas_penghargaan'] = $this->get_laporan->getpenghargaan();

            $this->load->view('templates/topbar', $data);
            $this->load->view('templates/sidebar_user', $data);
            $this->load->view('user/berkas_penghargaan', $data);
            $this->load->view('templates/footer');
        } else {
            $upload_file = $_FILES['file']['name'];
            if ($upload_file) {
                $config['allowed_types'] = 'xls|xlsx|doc|docx|ppt|pptx|pdf|zip|rar';
                $config['max_size']     = '51200';
                $config['upload_path'] = './assets/files/berkas_penghargaan/';
                $this->load->library('upload', $config);
                if ($this->upload->do_upload('file')) {
                    $old_file = $data['id']['file'];
                    if ($old_file != 'default.docx') {
                        unlink(FCPATH . 'assets/files/berkas_penghargaan/' . $old_file);
                    }
                    $new_file = $this->upload->data('file_name');
                    $data = array(
                        'id_user' => $this->input->post('id_user'),
                    ...
                    $this->db->insert('berkas_penghargaan', $data);
                    $this->session->set_flashdata('message', 'Upload data');
                    redirect('user/berkas_penghargaan');
                } else {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger font-weight-bolder" role="alert">UPLOAD GAGAL !..  Ekstensi File Salah / Ukuran file tidak boleh dari 10 mb</div>');
                    redirect('user/berkas_penghargaan');
                }
    ...
    public function edit_berkas_penghargaan($id)
    {
        $this->form_validation->set_rules('id', 'ID', 'required|trim');

        if ($this->form_validation->run() == FALSE) {
            $data['title'] = 'Edit Berkas Penghargaan';
            $data['user'] = $this->db->get_where('mst_user', ['username' => $this->session->userdata('username')])->row_array();
            $data['berkas_penghargaan'] = $this->db->get_where('berkas_penghargaan', ['id' => $id])->row_array();

            $this->load->view('templates/topbar', $data);
            $this->load->view('templates/sidebar_user', $data);
            $this->load->view('user/edit_berkas_penghargaan', $data);
            $this->load->view('templates/footer');
        } else {
            $upload_file = $_FILES['file']['name'];
            if ($upload_file) {
                ...
                $this->load->library('upload', $config);
                if ($this->upload->do_upload('file')) {
                    $old_file = $data['berkas_penghargaan']['file_upload'];
                    if ($old_file != 'default.docx') {
                        unlink(FCPATH . 'assets/files/berkas_penghargaan/' . $old_file);
                    }
                    $new_file = $this->upload->data('file_name');
                    ...
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger font-weight-bolder" role="alert">UPLOAD GAGAL !! Ekstensi File Salah / Ukuran file tidak boleh dari 10 mb</div>');
                    redirect('user/berkas_penghargaan/' . $id);
                }
            }
            $id = $this->input->post('id');
            $date_edit = $this->input->post('date_edit');
            ...
            $this->db->set('nama_file', $nama_file);
            $this->db->where('id', $id);
            $this->db->update('berkas_penghargaan');

            $this->session->set_flashdata('message', 'Update data');
            redirect('user/edit_berkas_penghargaan/' . $id);
        }
    }

    public function file_download_berkas_penghargaan($id)
    {
        $data = $this->db->get_where('berkas_penghargaan', ['id' => $id])->row_array();
        header("Content-Disposition: attachment; filename=" . $data['file_upload']);
        $fp = fopen("assets/files/berkas_penghargaan/" . $data['file_upload'], 'r');
        $content = fread($fp, filesize('assets/files/berkas_penghargaan/' . $data['file_upload']));
        fclose($fp);
        echo $content;
        exit;
    }

    public function del_berkas_penghargaan($id)
    {
        $_id = $this->db->get_where('berkas_penghargaan', ['id' => $id])->row();
        $query = $this->db->delete('berkas_penghargaan', ['id' => $id]);
        if ($query) {
            unlink("assets/files/berkas_penghargaan/" . $_id->file_upload);
        }
        $this->session->set_flashdata('message', 'Hapus data');
        redirect('user/berkas_penghargaan');
    }
    public function berkas_penilaiankerja()
    {
        $this->form_validation->set_rules('id_user', 'ID', 'required|trim');

        if ($this->form_validation->run() == false) {
            $data['title'] = 'Input Penilaian Kerja';
            $data['user'] = $this->db->get_where('mst_user', ['username' => $this->session->userdata('username')])->row_array();
            $data['berkas_penilaiankerja'] = $this->get_laporan->getpenilaiankerja();

            $this->load->view('templates/topbar', $data);
            $this->load->view('templates/sidebar_user', $data);
            $this->load->view('user/berkas_penilaiankerja', $data);
            $this->load->view('templates/footer');
        } else {
            $upload_file = $_FILES['file']['name'];
            if ($upload_file) {
                $config['allowed_types'] = 'xls|xlsx|doc|docx|ppt|pptx|pdf|zip|rar';
                $config['max_size']     = '51200';
                $config['upload_path'] = './assets/files/berkas_penilaiankerja/';
                $this->load->library('upload', $config);
                if ($this->upload->do_upload('file')) {
                    $old_file = $data['id']['file'];
                    if ($old_file != 'default.docx') {
                        unlink(FCPATH . 'assets/files/berkas_penilaiankerja/' . $old_file);
                    }
                    $new_file = $this->upload->data('file_name');
                    $data = array(
                        'id_user' => $this->input->post('id_user'),
                    ...
                    $this->db->insert('berkas_penilaiankerja', $data);
                    $this->session->set_flashdata('message', 'Upload data');
                    redirect('user/berkas_penilaiankerja');
                } else {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger font-weight-bolder" role="alert">UPLOAD GAGAL !..  Ekstensi File Salah / Ukuran file tidak boleh dari 10 mb</div>');
                    redirect('user/berkas_penilaiankerja');
                }
    ...
    public function edit_berkas_penilaiankerja($id)
    {
        $this->form_validation->set_rules('id', 'ID', 'required|trim');
        ...
        if ($this->form_validation->run() == FALSE) {
            $data['title'] = 'Edit Berkas Diklat';
            $data['user'] = $this->db->get_where('mst_user', ['username' => $this->session->userdata('username')])->row_array();
            $data['berkas_penilaiankerja'] = $this->db->get_where('berkas_penilaiankerja', ['id' => $id])->row_array();

            $this->load->view('templates/topbar', $data);
            $this->load->view('templates/sidebar_user', $data);
            $this->load->view('user/edit_berkas_penilaiankerja', $data);
            $this->load->view('templates/footer');
        } else {
            $upload_file = $_FILES['file']['name'];
            if ($upload_file) {
                ...
                $this->load->library('upload', $config);
                if ($this->upload->do_upload('file')) {
                    $old_file = $data['berkas_penilaiankerja']['file_upload'];
                    if ($old_file != 'default.docx') {
                        unlink(FCPATH . 'assets/files/berkas_penilaiankerja/' . $old_file);
                    }
                    $new_file = $this->upload->data('file_name');
                    ...
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger font-weight-bolder" role="alert">UPLOAD GAGAL !! Ekstensi File Salah / Ukuran file tidak boleh dari 10 mb</div>');
                    redirect('user/berkas_penilaiankerja/' . $id);
                }
            }
            $id = $this->input->post('id');
            $date_edit = $this->input->post('date_edit');
            ...
            $this->db->set('nama_file', $nama_file);
            $this->db->where('id', $id);
            $this->db->update('berkas_penilaiankerja');

            $this->session->set_flashdata('message', 'Update data');
            redirect('user/edit_berkas_penilaiankerja/' . $id);
        }
    }

    public function file_download_berkas_penilaiankerja($id)
    {
        $data = $this->db->get_where('berkas_penilaiankerja', ['id' => $id])->row_array();
        header("Content-Disposition: attachment; filename=" . $data['file_upload']);
        $fp = fopen("assets/files/berkas_penilaiankerja/" . $data['file_upload'], 'r');
        $content = fread($fp, filesize('assets/files/berkas_penilaiankerja/' . $data['file_upload']));
        fclose($fp);
        echo $content;
        exit;
    }

    public function del_berkas_penilaiankerja($id)
    {
        $_id = $this->db->get_where('berkas_penilaiankerja', ['id' => $id])->row();
        $query = $this->db->delete('berkas_penilaiankerja', ['id' => $id]);
        if ($query) {
            unlink("assets/files/berkas_penilaiankerja/" . $_id->file_upload);
        }
        $this->session->set_flashdata('message', 'Hapus data');
        redirect('user/berkas_penilaiankerja');
    }
